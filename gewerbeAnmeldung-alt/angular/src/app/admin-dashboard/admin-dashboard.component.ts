...
// wird ausgeführt wenn ein Antwort-Typ ausgewählt wird
  toggleEdit(){
    ...
    if(this.question.questionType.type == "Dropdown"){
      this.dropdownOn = true;
    }else{
      this.dropdownOn = false;
    }

    if(this.question.questionType.type == "Datum"){
      this.datumOn = true;
    }else{
      this.datumOn = false;
    }

    if(this.question.questionType.type == "Datei-Upload"){
      this.dateiUploadOn = true;
    }else{
      this.dateiUploadOn = false;
    }
...
getQuestionByIdForFrontend(id: number): string{
  ...
  //const result = this.hashIdQuestion.find( element => element.key === id );
  //console.log(result)
  //return "Moin";
  //return "ID: " + id;
}
/** 
  ...
  let getId = 'http://localhost:8090/frage/' + id;

  this.api.getQuestionbyId(getId).subscribe(dataZw => {console.log(dataZw);dataQuestion = dataZw;},err => {console.log(err);});
...
//___________________________________________________________________________________________________________________
  ...
  open(content) { 
    this.modalService.open(content, { windowClass : "my-modal-for-editNext"}).result.then((result) => {  // { size: 'lg', backdrop: 'static' }    { windowClass : "my-modal-for-editNext"}   {ariaLabelledBy: 'modal-basic-title'}
  ...
  open2(content) { 
    this.modalService.open(content, { windowClass : "my-modal-for-edit"}).result.then((result) => {  // { size: 'lg', backdrop: 'static' }    { windowClass : "my-modal-for-editNext"}   {ariaLabelledBy: 'modal-basic-title'}
      this.closeResult = `Closed with: ${result}`;
    }, (reason) => {
      this.closeResult = `Dismissed ${this.getDismissReason(reason)}`;
    });
...
export class NgbdModalBasic {
  ...
  open(content) {
    this.modalService.open(content, { windowClass: 'dark-modal' } ).result.then(   // { windowClass : "myCustomModalClass"}   {ariaLabelledBy: 'modal-basic-title'}
      (result) => {
